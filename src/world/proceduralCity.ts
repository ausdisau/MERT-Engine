...
export interface ProceduralCityPlan {
  seed: number;
...
function road(id: string, from: Vec3, to: Vec3): CityRoad {
  return { id, from, to, width: 14, walkableWidth: 10, gradient: 0.02, surfacePenalty: 0.02 };
}
...
export function generateProceduralCity(seed = 20260824): ProceduralCityPlan {
  ...
  for (const z of [-180,-90,0,90,180]) roads.push(road(`road-h-${z}`,[-230,0,z],[230,0,z]));
  for (const x of [-180,-90,0,90,180]) roads.push(road(`road-v-${x}`,[x,0,-230],[x,0,230]));
  ...
  const buildings: CityBuilding[] = [
    ...
    { id:'home-aisha',label:'Aisha home',kind:'home',position:[45,0,135],width:20,depth:18,height:7,districtId:'district-east-residential',residentIds:['aisha'],access:{automaticDoor:true,stepFreeEntry:true,charging:false,quietSpace:true} },
  ...
  const blockCentres: Array<[number,number]> = [[-135,135],[135,135],[-135,-45],[45,-45],[45,-135],[135,135],[-45,135]];
  for (let i=0;i<blockCentres.length;i++) {
    const centre = blockCentres[i];
    if (!centre) continue;
    const [x,z] = centre;
    const kind: CityBuildingKind = random() > 0.72 ? 'retail' : 'home';
    buildings.push({
      id:`generated-${i}`,
      label: kind === 'home' ? `Accessible residence ${i+1}` : `Neighbourhood service ${i+1}`,
      kind,
      position:[x + (random()-.5)*18,0,z + (random()-.5)*18],
      width:18 + random()*8,
      depth:16 + random()*8,
      height:6 + random()*5,
      districtId: x < 0 ? 'district-west-residential' : 'district-east-residential',
      residentIds:[],
      access:{automaticDoor:random()>.2,stepFreeEntry:true,charging:random()>.55,quietSpace:random()>.5},
    });
  }
...
function buildingEntity(building: CityBuilding): WorldEntity {
  const tags = ['city-building',building.kind,`district:${building.districtId}`];
  for (const resident of building.residentIds) tags.push(`resident:${resident}`);
  return { id:building.id,kind:'place',label:building.label,position:building.position,persistent:true,tags };
}

function entranceEntity(building: CityBuilding): InfrastructureEntity {
  return {
    id:`${building.id}-entry`,
    kind:'infrastructure',
    label:`${building.label} entrance`,
    position:[building.position[0],0,building.position[2]-building.depth/2-2],
    persistent:true,
    tags:['city-entry',building.kind],
    feature: building.access.automaticDoor ? 'automatic-door' : 'ramp',
    operational:true,
    accessibility:['walk','powered-wheelchair','manual-wheelchair','touch'],
  };
}

export function applyProceduralCity(store: PersistentWorldStore, plan: ProceduralCityPlan) {
  for (const building of plan.buildings) {
    if (!store.entity(building.id)) store.upsertEntity(buildingEntity(building) as OpenWorldEntity);
    const entry = entranceEntity(building);
    if (!store.entity(entry.id)) store.upsertEntity(entry);
    if (building.access.charging) {
      const charge: InfrastructureEntity = {
        id:`${building.id}-charge`,kind:'infrastructure',label:`${building.label} accessible charging`,
        position:[building.position[0]+4,0,building.position[2]+4],persistent:true,tags:['city-charge',building.kind],
        feature:'charging-point',operational:true,accessibility:['powered-wheelchair','manual-wheelchair','touch'],
      };
      if (!store.entity(charge.id)) store.upsertEntity(charge);
    }
  }
  store.recordEvent('city.generated',undefined,`seed=${plan.seed}; districts=${plan.districts.length}; roads=${plan.roads.length}; buildings=${plan.buildings.length}`);
}
