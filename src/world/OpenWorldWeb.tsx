...
export function OpenWorldWeb() {
  ...
  useEffect(()=>{
    ...
    const boot=async()=>{
      ...
      scene=new B.Scene(engine);scene.clearColor=new B.Color4(.72,.86,.96,1);scene.collisionsEnabled=true;
      ...
      const makeBuilding=(id:string,label:string,x:number,z:number,w:number,d:number,h:number)=>{const body=B.MeshBuilder.CreateBox(label,{width:w,depth:d,height:h},scene);body.position.set(x,h/2,z);body.material=buildingMat;body.checkCollisions=true;const door=B.MeshBuilder.CreateBox(id,{width:3.2,depth:.12,height:2.7},scene);door.position.set(x,1.35,z-d/2-.07);door.material=glass;doorMeshes.set(id,{mesh:door,closedX:x});};
      makeBuilding('clinical-door','Clinical Simulation Centre',-24,8,28,20,10);makeBuilding('community-door','Community Hub',25,22,26,18,7);makeBuilding('rehab-door','Rehabilitation Lab',24,-28,24,18,8);makeBuilding('transit-door','Accessible Transit Station',-25,-34,30,15,6);
      ...
      setStatus('WOLFRAM-GROUNDED OPEN WORLD ONLINE');engine.runRenderLoop(()=>scene.render());const resize=()=>engine.resize();window.addEventListener('resize',resize);(scene as any).__cleanup=()=>window.removeEventListener('resize',resize);
    };
    boot();return()=>{disposed=true;scene?.__cleanup?.();routeMesh?.dispose?.();scene?.dispose?.();engine?.dispose?.();};
  },[]);
  ...
  return <div style={{position:'fixed',inset:0,background:'#111'}}><canvas ref={canvasRef} aria-label="Interactive multi-person disability-inclusive open world with accessible route planning, working doors, lift, charging, transport, social encounters and clinical continuity. Desktop uses WASD and mouse; compatible WebXR headsets can enter VR." style={{width:'100%',height:'100%',touchAction:'none',display:'block'}}/><div role="status" aria-live="polite" style={{position:'absolute',top:16,left:16,maxWidth:760,maxHeight:'72vh',overflow:'auto',padding:'12px 14px',borderRadius:12,background:'rgba(8,18,28,.92)',color:'white',fontFamily:'system-ui'}}><strong>MERT · WOLFRAM OPEN WORLD · STEPS 21–24</strong><br/><span>{status}</span><br/><span>{xr}</span><br/><span>{access}</span><br/><span>{life}</span><br/><span>{route}</span><br/><span>{objects}</span><br/><span>{nearby}</span><br/><span>{encounters}</span><br/><span>{clinical}</span><br/><span>{continuity}</span><div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}><button onClick={interactNearest} style={{padding:10}}>Interact with nearest</button><button onClick={openDoor} style={{padding:10}}>Open Community door</button><button onClick={moveLift} style={{padding:10}}>Move station lift</button><button onClick={chargeAAC} style={{padding:10}}>Charge Maya AAC</button><button onClick={boardTransport} style={{padding:10}}>Board shuttle</button><button onClick={requestSupport} style={{padding:10}}>Request Liam support</button><button onClick={beginContinuity} style={{padding:10}}>Start continuity episode</button><button onClick={advanceContinuity} style={{padding:10}}>Advance care journey</button><button onClick={toggleLift} style={{padding:10}}>Toggle lift availability</button><button onClick={toggleAAC} style={{padding:10}}>Toggle Maya AAC</button><button onClick={resetWorld} style={{padding:10}}>Reset world</button></div></div><div style={{position:'absolute',right:16,bottom:16,maxWidth:470,padding:12,borderRadius:12,background:'rgba(8,18,28,.88)',color:'white',fontFamily:'system-ui',fontSize:13}}><strong>Wolfram computation layer</strong><br/>Route cost: {WOLFRAM_ROUTE_MODEL.formula}. Lift: {WOLFRAM_OBJECT_MODELS.liftSmoothstep}. Charger: {WOLFRAM_OBJECT_MODELS.charger}. Social encounters: {WOLFRAM_SOCIAL_MODEL.formula}. Continuity graph verified acyclic with {WOLFRAM_CONTINUITY_MODEL.unexpectedDeadEnds} unexpected dead ends. Simulation parameters are educational model values, not accessibility-code or clinical-treatment thresholds.</div></div>;
}
