import type { AccessModality, InfrastructureEntity, PersonEntity, PersistentWorldState, Vec3 } from './worldModel';
...
export interface NavEdge { from:string; to:string; length:number; gradient:number; surfacePenalty:number; waitSeconds:number; feature:RouteFeature; access:AccessModality[]; objectId?:string; }
export interface NavigationProfile { mobility:AccessModality; maxGradient:number; gradientWeight:number; blockedPenalty:number; }
export interface RouteResult { nodeIds:string[]; points:Vec3[]; cost:number; features:RouteFeature[]; blockedBy:string[]; }
export interface NavigationNetwork { nodes:NavNode[]; edges:NavEdge[]; }

export const defaultNavigationNetwork:NavigationNetwork={
  nodes:[
    {id:'plaza',label:'Central plaza',position:[0,0,0]},{id:'community-door',label:'Community automatic door',position:[14,0,14]},{id:'community-hub',label:'Community Hub',position:[25,0,22]},
    {id:'clinical-ramp',label:'Clinical ramp',position:[-11,0,4]},{id:'clinical-door',label:'Clinical automatic door',position:[-18,0,8]},{id:'clinical-centre',label:'Clinical Simulation Centre',position:[-24,0,8]},
    {id:'crossing',label:'Accessible crossing',position:[-8,0,-15]},{id:'station-lift',label:'Station lift',position:[-20,0,-34]},{id:'transit',label:'Accessible Transit Station',position:[-25,0,-34]},
    {id:'rehab-ramp',label:'Rehabilitation ramp',position:[10,0,-13]},{id:'rehab-lab',label:'Rehabilitation Lab',position:[24,0,-28]},
  ],
  edges:[
...
export function navigationProfile(person:PersonEntity):NavigationProfile{const mobility=person.mobility.mode;const maxGradient=mobility==='manual-wheelchair'?.07:mobility==='powered-wheelchair'?.09:.16;return{mobility,maxGradient,gradientWeight:4,blockedPenalty:1_000_000};}
function edgeOperational(edge:NavEdge,state:PersistentWorldState){if(!edge.objectId)return true;const entity=state.entities.find(item=>item.id===edge.objectId);if(!entity||entity.kind!=='infrastructure')return true;return (entity as InfrastructureEntity).operational;}
export function edgeCost(edge:NavEdge,person:PersonEntity,state:PersistentWorldState){const profile=navigationProfile(person);if(!edge.access.includes(profile.mobility))return Infinity;if(Math.abs(edge.gradient)>profile.maxGradient)return Infinity;if(!edgeOperational(edge,state))return Infinity;return edge.length*(1+profile.gradientWeight*Math.abs(edge.gradient)+edge.surfacePenalty)+edge.waitSeconds;}
function adjacency(network:NavigationNetwork){const map=new Map<string,Array<{edge:NavEdge;next:string}>>();for(const edge of network.edges){map.set(edge.from,[...(map.get(edge.from)??[]),{edge,next:edge.to}]);map.set(edge.to,[...(map.get(edge.to)??[]),{edge,next:edge.from}]);}return map;}
export function nearestNavNode(network:NavigationNetwork,position:Vec3){return [...network.nodes].sort((a,b)=>Math.hypot(a.position[0]-position[0],a.position[2]-position[2])-Math.hypot(b.position[0]-position[0],b.position[2]-position[2]))[0];}
export function findAccessibleRoutes(network:NavigationNetwork,state:PersistentWorldState,person:PersonEntity,startId:string,goalId:string,limit=3):RouteResult[]{const adj=adjacency(network),results:RouteResult[]=[],maxDepth=network.nodes.length;const visit=(node:string,path:string[],cost:number,features:RouteFeature[])=>{if(path.length>maxDepth||results.length>200)return;if(node===goalId){const points=path.map(id=>network.nodes.find(n=>n.id===id)?.position).filter((point):point is Vec3=>Boolean(point));if(points.length===path.length)results.push({nodeIds:path,points,cost,features,blockedBy:[]});return;}for(const item of adj.get(node)??[]){if(path.includes(item.next))continue;const c=edgeCost(item.edge,person,state);if(!Number.isFinite(c))continue;visit(item.next,[...path,item.next],cost+c,[...features,item.edge.feature]);}};visit(startId,[startId],0,[]);return results.sort((a,b)=>a.cost-b.cost).slice(0,limit);}
export function routeForPerson(state:PersistentWorldState,person:PersonEntity,targetId:string,network=defaultNavigationNetwork){const start=nearestNavNode(network,person.position);const targetPosition=state.entities.find(e=>e.id===targetId)?.position;const goal=network.nodes.find(node=>node.id===targetId)??(targetPosition?nearestNavNode(network,targetPosition):undefined);return start&&goal?findAccessibleRoutes(network,state,person,start.id,goal.id,3):[];}
